...
public:
		...
		if (sConfig->GetBoolDefault("Npc_Buff.OnlyGMs", false)) // If Npc_Buff.OnlyGMs is enabled in worldserver.conf
			if (pPlayer->GetSession()->GetSecurity() == SEC_PLAYER) {
				pCreature->MonsterWhisper(
						"Sorry, I can only Buff Platinum Members.",
						pPlayer->GetGUID());
				return true;
			}

		bool EnableSmallBuff = sConfig->GetBoolDefault(
				"Npc_Buff.EnableSmallBuff", true);
		bool EnableGreatBuff = sConfig->GetBoolDefault(
				"Npc_Buff.EnableGreatBuff", true);
		bool EnableGMBuff = sConfig->GetBoolDefault("Npc_Buff.EnableGMBuff",
				true);
		bool EnablePlayerTools = sConfig->GetBoolDefault(
				"Npc_Buff.EnablePlayerTools", true);
		bool EnableResSickness = sConfig->GetBoolDefault(
				"Npc_Buff.EnableRemoveResSickness", true);
		bool EnableGivemeGold = sConfig->GetBoolDefault(
				"Npc_Buff.EnableGivemeGold", false);
	...
	bool showSmallBuff(Player *pPlayer, Creature *pCreature,
			uint32 showFromId = 0) {

		QueryResult result;
		result
				= WorldDatabase.PQuery(
						"SELECT `name`, `cat_number` FROM `npc_buff_spells` WHERE `cat_number` < 2000 ORDER BY `cat_number` ASC");
		...
		if (result) {
			...
			if (showFromId = 0) {
				//you are too poor
				pCreature->MonsterWhisper("You don't have enough money.",
						pPlayer->GetGUID());
	...
	bool showGreatBuff(Player *pPlayer, Creature *pCreature,
			uint32 showFromId = 0) {

		QueryResult result;
		result
				= WorldDatabase.PQuery(
						"SELECT `name`, `cat_number` FROM `npc_buff_spells` WHERE `cat_number` < 3000 AND `cat_number` > 2000 ORDER BY `cat_number` ASC");
	...
	bool showGmBuff(Player *pPlayer, Creature *pCreature, uint32 showFromId = 0) {
		...
		result
				= WorldDatabase.PQuery(
						"SELECT `name`, `cat_number` FROM `npc_buff_spells` WHERE `cat_number` < 4000 AND `cat_number` > 3000 ORDER BY `cat_number` ASC");
	...
	bool showPlayerTools(Player *pPlayer, Creature *pCreature,
			uint32 showFromId = 0) {

		QueryResult result;
		result
				= WorldDatabase.PQuery(
						"SELECT `name`, `cat_number` FROM `npc_buff_spells` WHERE `cat_number` < 5000 AND `cat_number` > 4000 ORDER BY `cat_number` ASC");
	...
	void SendDefaultMenu(Player* pPlayer, Creature* pCreature, uint32 uiAction) {
		...
		result
				= WorldDatabase.PQuery(
						"SELECT `spell_id`, `cat_number`, `cost`, `name` FROM `npc_buff_spells` WHERE `cat_number` = %u LIMIT 1",
						uiAction);
		...
		if (result) {
			...
			do {
				...
				if (pPlayer->GetMoney() < goldCost) {
					pCreature->MonsterWhisper("You dont have enough money!",
							pPlayer->GetGUID());
					pPlayer->CLOSE_GOSSIP_MENU();
					return;
				} else if (uiAction < 5000 && uiAction != 1000 && uiAction
						!= 2000 && uiAction != 3000 && uiAction != 4000
						&& uiAction != 5005) {
					pPlayer->CLOSE_GOSSIP_MENU();
		...
		case 5000://Remove Res Sickness
			if (!pPlayer->HasAura(SPELL_RESURRECTION_SICKNESS_15007, 0)) {
				pCreature->MonsterWhisper(
						"You don't have resurrection sickness.",
						pPlayer->GetGUID());
